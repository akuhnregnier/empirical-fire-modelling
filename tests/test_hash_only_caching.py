...
def test_hash_only_caching(test_hash_only_cache):
    @test_hash_only_cache
    def f(x):
        """Function to cache."""
        return x + 1
...
def test_hash_only_caching_tuple(test_hash_only_cache):
    @test_hash_only_cache
    def f(x):
        """Function to cache."""
        return (x + 1, x + 2)
    ...
    assert all(custom_get_hash(v) == cached_hash_values[i] for i, v in enumerate(out))
